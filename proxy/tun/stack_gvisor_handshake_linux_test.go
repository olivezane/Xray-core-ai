...
// This test drives real packets through the gVisor Stack right after it starts,
// and reports the marker the gVisor Stack puts on the wire for a handshake, so
// the MIPS Stack's own answer (TestMipstackAnswersSYNWithSYNACK) can be told
// apart from it in a real capture.
...
func TestGVisorStackAnswersSYNWithSYNACK(t *testing.T) {
	...
	device := &fakeFDTun{fds: []int{pair[1]}, mtu: handshakeMTU}

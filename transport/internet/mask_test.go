...
// The mask-before-TLS wrap-order guard lives in transport/internet/security:
// the TLS step is applied there, behind WrapConnClient.
func TestWrapConnClientNoMaskPassthrough(t *testing.T) {
	client, server := net.Pipe()
	defer client.Close()
	defer server.Close()

	dest := cnet.TCPDestination(cnet.LocalHostIP, 443)
	for _, mss := range []*internet.MemoryStreamConfig{nil, {}} {
		result, err := security.WrapConnClient(mss, context.Background(), dest, client, &security.SecurityHooks{})
		if err != nil {
			t.Fatal(err)
		}
		if result != client {
			t.Fatalf("mss=%v: expected raw conn passthrough, got %v", mss, result)
		}
	}
}
...
func TestWrapConnClientMaskErrorClosesRaw(t *testing.T) {
	...
	_, err := security.WrapConnClient(mss, context.Background(), cnet.TCPDestination(cnet.LocalHostIP, 443), raw, nil)
